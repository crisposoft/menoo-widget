...
#### `setLanguage(language: 'ro' | 'en' | 'ru'): Promise<void>`

Change widget language dynamically.

```javascript
await MenooSDK.setLanguage("en");
```
...
```javascript
const cart = MenooSDK.getCart();
console.log(cart.totalPrice, cart.totalCount);
```
...
```javascript
const count = MenooSDK.getItemCount();
```
...
Programmatically open checkout flow.
...
Listen to widget events for custom integrations.

### `menoo:ready`

Fired when widget is initialized and ready.

```javascript
window.addEventListener("menoo:ready", (e) => {
  console.log("Widget ready:", e.detail.restaurantId);
});
```
...
window.addEventListener("menoo:checkout-started", (e) => {
  console.log("Checkout started:", e.detail.cart);
});
```

### `menoo:order-completed`

Fired when order is successfully completed.

```javascript
window.addEventListener("menoo:order-completed", (e) => {
  console.log("Order completed:", e.detail.order);
});
```
...
```css
#menoo-widget {
  --menoo-primary: #your-brand-color;
  --menoo-surface: #ffffff;
  --menoo-background: #f5f5f5;
  --menoo-text-primary: #212121;
  --menoo-text-secondary: #757575;
  --menoo-radius-md: 8px;
  --menoo-spacing-2: 16px;
}
```
